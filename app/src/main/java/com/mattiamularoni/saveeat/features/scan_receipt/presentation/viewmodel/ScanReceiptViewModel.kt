...
class ScanReceiptViewModel(
    ...
    fun analyzeReceipt(bitmap: Bitmap) {
        ...
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = { items -> onItemsParsed(items) },
                onFailure = { exception ->
    ...
    private suspend fun onItemsParsed(items: List<ParsedReceiptItem>) {
        val (perishable, nonPerishable) = items.partition { it.isPerishable }
        ...
        if (nonPerishable.isNotEmpty()) {
            ...
            runCatching { pantryRepository.saveReceiptItems("", pantryItems) }
